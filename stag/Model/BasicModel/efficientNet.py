...
def get_efficientnet_v2_structure(model_name):
    if 'efficientnet_v2_xl' in model_name:
        return [
...
class SEUnit(nn.Module):
    ...
    """
    def __init__(self, in_channel, reduction_ratio=4, act1=partial(nn.GELU), act2=nn.Sigmoid):
...
class MBConvConfig:
    ...
    def __init__(self, expand_ratio: float, kernel: int, stride: int, in_ch: int, out_ch: int, layers: int,
                 use_se: bool, fused: bool, act=nn.GELU, norm_layer=nn.BatchNorm2d):
...
class EfficientNetV2(nn.Module):
    ...
    def __init__(self, layer_infos, out_channels=1280, dropout=0.2, stochastic_depth=0.0,
                 block=MBConv, act_layer=nn.GELU, norm_layer=nn.BatchNorm2d):
        ...
        self.stem = ConvBNAct(1, self.in_channel, 3, 2, 1, self.norm_layer, self.act)
...
def get_efficientnet_v2(dropout=0.2, stochastic_depth=0, **kwargs):
    residual_config = [MBConvConfig(*layer_config) for layer_config in get_efficientnet_v2_structure('efficientnet_v2_l')]
    model = EfficientNetV2(residual_config, 1280, dropout=dropout, stochastic_depth=stochastic_depth, block=MBConv, act_layer=nn.GELU)
