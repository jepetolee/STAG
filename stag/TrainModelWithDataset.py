...
def TrainWithDataset(crypto_name, load_model = False,adder=0,device='cuda', train_steps_per_trade=10):
    ...
    for number in range(len(testing_number_data)-adder -3200+1):
        decided_number =number+adder
        url = 'G:/' + crypto_name + '/COMBINED/' + str(decided_number + 1) + '.png'
        ...
        close_price_in_csv_data = testing_number_data['ClosePrice'][LeastNumber2Build + 3000 + decided_number]
...
if __name__ == '__main__' :
    ...
    TrainWithDataset('BTCUSDT',load_model=True,adder =70000)
