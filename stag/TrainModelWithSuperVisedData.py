...
def calculate_accuracy(predictions, ground_truth):
    ...
    if len(predictions) != len(ground_truth):
        raise ValueError("예측값과 실제값의 길이가 일치해야 합니다.")
    ...
    for epoch in range(1, epoch+1):
        ...
        with tqdm(iter(train_loader)) as pbar:
            ...
            for imgs, labels in pbar:
                ...
                model.zero_grad()
                output = model(imgs)
                output_array.append(torch.argmax(output, dim=1))
                answer_array.append(labels)
        ...
        if best_train_loss > _train_loss:
            ...
            torch.save(model.state_dict(), './BestTrain.pt')
        _val_loss = validation(model, criterion, val_loader, device)


        lr =optimizer.param_groups[0]['lr']
        print(calculate_accuracy(torch.cat(output_array, dim=0), torch.cat(answer_array, dim=0))*100)
...
train(model, optimizer, train_loader, val_loader, 'cuda', CFG['EPOCHS'], './pretrained.pt',True)